...
def get_reviews_from_db():
    try:
        ...
        sql = """
            SELECT f.rating, f.comment, f.feedback_date, u.username
            FROM "Feedback" f
        ...
        for row in rows:
            formatted_reviews.append({
                'name': row.get('username', 'Người dùng ẩn'),
                'rating': row.get('rating', 5),
                'text': row.get('comment', ''),
                'date': row.get('feedback_date'), # Lưu ý: DB có thể trả về object Date hoặc String
                'avatar_bg': random.choice(colors) # Random màu vì DB không lưu cái này
            })
...
def submit_review():
    """API lưu feedback vào PostgreSQL"""
    ...
    try:
        ...
        db = get_db_connection()
        sql = """
            INSERT INTO "Feedback" (user_id, rating, comment, feedback_date)
            VALUES (%s, %s, %s, %s)
        """
        # Hàm execute trả về True/False
        success = db.execute(sql, (user_id, rating, comment, now))
